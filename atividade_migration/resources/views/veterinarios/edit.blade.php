@extends('templates.main', ['titulo' => "Editar Veterinario"])
    ...
    <div class="row">
        <div class="col">
            <div class="input-group mb-3">
                <label class="input-group-text" for="inputGroupSelect01" color="#0d6efd">Especialidade</label>
                <select name="especialidade_id" class="form-control">
                    @foreach($esp as $chave)
                        <option value="{{$chave->id}}" @if($chave->id == $dados['especialidade_id']) selected="true" @endif>
                            {{$chave->nome}}
                        </option>
                    @endforeach
                </select>
